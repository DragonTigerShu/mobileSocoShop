...
/*.animation-ease {
 ...
 .swiper-container-vertical > .slider-pagination-bullets {
  ...
  right: 10px;
  margin-bottom:0px;
  transform: translate3d(88%, -50%, 0px);
}
  ...
  <div class='slider-container' :class = 'basicdata.containerClass'>
    ...
    <template  v-if="!sliderinit.loop">
      <template  v-for="item in pages">
        <div class="slider-item" :style="'background:url('+baseUrl+item.pic_ori+') center center no-repeat'">
          {{item.title}}
        </div>
      </template>
    </template>
    ...
    <template  v-if="sliderinit.loop">
     ...
     <template v-if="sliderinit.infinite" v-for="(item, index) in pages" >
      <div v-if="index+sliderinit.infinite-pages.length>=0" class="slider-item" :style="'background:url('+baseUrl+item.pic_ori+') center center no-repeat'">{{item.title}}</div>
    </template>
    <template v-if="!sliderinit.infinite">
      <div class="slider-item"  :style="'background:url('+baseUrl+pages[pages.length-1].pic_ori+') center center no-repeat'" >{{pages[pages.length-1].title}}</div>
    </template>
    <!-- 无缝滚动copy最后项 -->
    <template  v-for="item in pages">
      <div class="slider-item" :style="'background:url('+baseUrl+item.pic_ori+') center center no-repeat'">{{item.title}}</div>
    </template>
    <!-- 无缝滚动copy第一项 -->
    <template v-if="sliderinit.infinite" v-for="(item, index) in pages" >
      <div v-if="index-sliderinit.infinite<0" class="slider-item" :style="'background:url('+baseUrl+item.pic_ori+') center center no-repeat'">{{item.title}}</div>
    </template>
    <template v-if="!sliderinit.infinite">
      <div class="slider-item" :style="'background:url('+baseUrl+pages[0].pic_ori+') center center no-repeat'">{{pages[0].title}}</div>
    </template>
  </template>
</div>
<div class="slider-pagination slider-pagination-bullets">
  <template  v-for="n in pagenums">
   <span @click='slide(n-1)' class="slider-pagination-bullet" :class="n-1 == sliderinit.currentPage? 'slider-pagination-bullet-active':''">
    <i :class="n-1 == sliderinit.currentPage? 'slider-pagination-active':'slider-pagin'">{{pages[n-1].title}}</i>
  </span>
...
<script>
  ...
  export default {
   props: ['sliderinit', 'pages','baseUrl'],
   data(){
    return {
      basicdata:{
        poswidth:'0',
        ...
        pageslist:['socotc','socots','socolite'],
        tracking: false,
