...
(function($, window, document, name) {
	...
	var OffCanvas = $.Class.extend({
				...
				case 'drag':
					...
					if (this.isDragging) {
						if(this.offsetX>0){
							...
							this.updateTranslate(this.offCanvasLeftWidth);
						}
						// this.updateTranslate(this.offsetX);
		...
		isShown: function(direction) {
			var shown = false;
			document.querySelector('.mui-inner-wrap').classList.remove('mui-transform');  // 去掉transform
			if (!this.slideIn) {
				var x = this.getTranslateX();
				if (direction === 'right') {
		...
		close: function() {
			...
			document.querySelector('.mui-inner-wrap').classList.add('mui-transform') // 添加transform
		},
